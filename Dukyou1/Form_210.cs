...
namespace Dukyou3
{
    public partial class Form_210 : Form
    {
        ...
        public void Grid_View(string Query)
        {
            ...
            grid1[Rows, 10] = new MyHeader1("쇼핑몰 코드");
            grid1[Rows, 11] = new MyHeader("낱장ID");
            ...
            while (myRead.Read())
            {
                ...
                grid1[Rows, 10] = new SourceGrid.Cells.Cell(myRead["shop_code"], typeof(string));
                grid1[Rows, 10].View = cc.left_cellr;

                grid1[Rows, 11] = new SourceGrid.Cells.Cell(myRead["nat_jang_id"].ToString() == "True" ? "1":"0", typeof(string));
                grid1[Rows, 11].View = cc.center_cellr;
                Rows++;
            }
        ...
        void GridNewLine(string row_no, string GetCode, string GetClass)
        {
            ...
            grid1[Rows, 10].View = cc.center_cellr;
        }
        ...
        private void ValueChangedEvent1(object sender, EventArgs e)  //Grid1에서 볼륨첸지
        {
            ...
            if (cpos.Equals(3))       //
                ...
                cQuery = " update " + DB_TalbeName_hang + " set shop_code='" + dat + "', mod_time=now() where row_id='" + row_no + "'";
            else if (cpos.Equals(11))  //
                cQuery = " update " + DB_TalbeName_hang + " set nat_jang_id='" + dat + "', mod_time=now() where row_id='" + row_no + "'";
