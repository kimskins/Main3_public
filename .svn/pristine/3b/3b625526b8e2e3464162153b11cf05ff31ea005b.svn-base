...
namespace Dukyou3
{
    public partial class Form_4012b : Form
    {
        ...
        public Form_4012b(string row_id, Form_4012 mom)
        {
            InitializeComponent();
            hcust_row_id = row_id;
            this.mom = mom;
        ...
        private void Form_4011b_Load(object sender, EventArgs e)
        {
            ...
            string Query = "select * from " + DB_TableName_hcust + " where row_id = '" + hcust_row_id + "'";
            ...
            if (myRead.Read())
            {
                tbHomePage.Text = myRead["homp_ip"].ToString();
                tbHomePage.Refresh();

                tbEmail.Text = "";
                tbEmail.Refresh();

                tbSangho.Text = myRead["sangho"].ToString();
                tbSangho.Refresh();

                tbCEO.Text = myRead["mast"].ToString();
                tbCEO.Refresh();

                tbHP.Text = myRead["mh_tel"].ToString();
                tbHP.Refresh();

                tbTel.Text = myRead["ctel"].ToString();
                tbTel.Refresh();
                ...
                tbJongMok.Text = gubun_temp;
                tbJongMok.Refresh();

                tbFax.Text = myRead["fax"].ToString();
                tbFax.Refresh();

                tbComNum.Text = myRead["enter_no"].ToString();
                tbComNum.Refresh();

                tbBirth.Text = myRead["in_day"].ToString();
                tbBirth.Refresh();

                tbAddress.Text = myRead["c_addr_new"].ToString();
                tbAddress.Refresh();
