...
namespace Dukyou3
{
    public partial class Form_3071 : Form
    {
        const string flag_id = "0";       //1이면 전체견적. 0이면 %견적으로 코딩될 예정
        ...
        const int start_grade_column = 5;   // 등급 전까지 컬럼 갯수
        int grade_count = 0;
        string customer = "";
        ...
        private void Form_309_Load(object sender, EventArgs e)
        {
        ...
        private void Grid_View()
        {
            ...
            string cQuery = "select * from " + DB_TableName_gyeon;
            cQuery += " where flag = '" + flag_id + "' order by sales_min, sales_max, row_id";
            // 등급 테이블에서 등급 갯수 Get
            ...
            //// grid 사이즈 조절
            //int grid_width = 192 + (grade_count * 70) + 20;
            //if (grid_width > 1400)
            //    grid_width = 1400;
            ...
            grid1[0, 3] = new MyHeader1("매출액(구간)");
            grid1[0, 3].ColumnSpan = 2;


            if (grade_count != 0)
            {
                grid1[0, 5] = new MyHeader1("등급");
                grid1[0, 5].ColumnSpan = grade_count;
            }
            ...
            while (myRead.Read())
            {
                ...
                grid1[m, 3] = new SourceGrid.Cells.Cell(myRead["sales_min"], typeof(string));
                grid1[m, 3].View = cc.int_cell;
                grid1[m, 3].Editor = cc.disable_cell;

                grid1[m, 4] = new SourceGrid.Cells.Cell(myRead["sales_max"], typeof(string));
        ...
        private void bMody_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 3; i < grid1.RowsCount; i++)
            {
                ...
                {
                    chk_num++;
                    row_id = Convert.ToInt32(grid1[i, 0].Value.ToString());
                }
            }
            if (chk_num > 1)
            {
        ...
        {
            Form_3072a Frm = new Form_3072a(flag_id); // 1은 전체견적, 0는 % 견적
            Frm.ShowDialog();
            Grid_View();
        }
        ...
        {
            Form_3072b Frm = new Form_3072b(1, flag_id, DB_TableName_gyeon);
            Frm.ShowDialog();
        ...
        {
            GridHandle.ChkDataDelete(DB_TableName_gyeon, 3, 0, 1);
            Grid_View();
        }
        ...
        private void bAutoInsert_Click(object sender, EventArgs e)
        {
            ...
            Form_3071d Frm = new Form_3071d(start_grade_column, grid1, chk_num, chk_rows);
            Frm.ShowDialog();
